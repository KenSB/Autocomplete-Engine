...
class LetterAutocompleteEngine:
        ...
        with open(config['file'], encoding='utf8') as f:
            for line in f:
                ...
                if has_alpha:
                    for c in s:
                        lst_line.append(c)
                    self.autocompleter.insert(s, 1.0, lst_line)
    ...
    def autocomplete(self, prefix: str,
        ...
        for c in prefix:
            lst_prefix.append(c)
        return self.autocompleter.autocomplete(lst_prefix, limit)
    ...
    def remove(self, prefix: str) -> None:
        ...
        for c in prefix:
            lst_prefix.append(c)
        self.autocompleter.remove(lst_prefix)
...
class SentenceAutocompleteEngine:
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        *Skip lines that do not contain at least one alphanumeric character!*
        ...
        with open(config['file']) as csvfile:
            ...
            for line in reader:
                side = line[0].lower().rstrip()
                s = line[0].lower().rstrip()
                for char in side:
                    if char not in alpha_num:
                        s = s.replace(char + '', '')
                if len(s) != 0:
                ...
                if has_alpha:
                    lst_line = s.split(' ')
                    self.autocompleter.insert(s, float(line[1]), lst_line)
    ...
    def autocomplete(self, prefix: str,
        ...
        if limit is None:
            lit = False
        lst_prefix = prefix.split(" ")
        store = self.autocompleter.search_for_prefix(lst_prefix)
        if store is not None:
            leaves = store.search_for_leaves()
            if lit:
                leaves = leaves[:limit]
            SentenceAutocompleteEngine.sort_leaves_helper(leaves)
            return leaves
        return []

    @staticmethod
    def sort_leaves_helper(leaves: List)-> None:
        """
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        with open(config['file']) as csvfile:
            ...
            for line in reader:
                ...
                index1 = 1
                index2 = 2
                while len(line) > index1:
                    notes.append((int(line[index1]), int(line[index2])))
                    index1 += 2
                    index2 += 2
                mel = Melody(name, notes)
                index3 = 0
                while len(notes) > index3 + 1:
                    prefix.append(notes[index3][0] - notes[index3 + 1][0])
                    index3 += 1
                self.autocompleter.insert(mel, 1.0, prefix)
...
def sample_letter_autocomplete() -> List[Tuple[str, float]]:
    ...
    return engine.autocomplete('no', 20)
